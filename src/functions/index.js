...
exports.onNewComic = functions.firestore.document('comics/{id}').
  onCreate(event => {
    ...
    return admin.firestore().
      ...
      then(chunks => Promise.all(
        chunks.map(chunk => admin.messaging().sendToDevice(chunk, payload)))).
      then(results => results.forEach((res, pos) => {

        console.log(`Batch ${pos +
        1} notifications sent. Failure Count: ${res.failureCount}`)
...
exports.onNewDeviceToken = functions.firestore.document('tokens/{uid}').
  onWrite(event => {
